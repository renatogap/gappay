...
CREATE TABLE IF NOT EXISTS usuario_sistema (
    ...
    ultimo_acesso date DEFAULT timezone('America/Belem', now()),
    status INT DEFAULT 1,
    ...
    FOREIGN KEY (fk_usuario_cadastro) REFERENCES usuario(id),
    FOREIGN KEY (fk_usuario_edicao) REFERENCES usuario(id)
);
...
(
    ...
    usuario_id  bigint    not null
        constraint usuario_log_usuario_id_foreign
            references usuario,
    responsavel bigint       not null
        constraint usuario_log_responsavel_foreign
            references usuario,
    sistema_id  bigint       not null
        constraint usuario_log_sistema_id_foreign
