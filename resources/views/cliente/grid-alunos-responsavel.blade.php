...
@section('conteudo')
    ...
    <div class="home-container">
        ...
        @if (session('success'))
            <div class="card-panel green lighten-4"
                style="color: #1b5e20; font-size: 14px; background-color: #ccf5d8 !important; padding: 10px; border-radius: 5px;">
                {{ session('success') }}
            </div>
        @endif
        ...
        @if (session('error'))
            <div class="card-panel red lighten-4"
                style="color: #b71c1c; font-size: 14px; background-color: #ffcdd2 !important; padding: 10px; border-radius: 5px;">
                {{ session('error') }}
            </div>
        @endif

        <a href="{{ url('cliente/aluno/create') }}" class="btn btn-primary btn-block mt-3 mb-3">
            <span class="material-icons" style="font-size:1em;vertical-align:middle;">add</span>
            Adicionar Novo Aluno
        </a>

        @forelse($alunos as $aluno)
            <div class="aluno-card" onclick="editarDadosAluno(event, '{{ $aluno->id }}')">
                <div class="avatar">
                ...
                <div class="actions">
                    <form action="{{ url('cliente/aluno/' . $aluno->id . '/delete') }}" method="POST" class="form-excluir">
                        @csrf @method('DELETE')
                        <button type="button" class="btn-action btn-del text-danger" data-value="deletar" title="Excluir"
                            onclick="abrirModalExcluir(this, '{{ $aluno->nome }}')">
                            <span class="material-icons" style="font-size:1.1em;" data-value="deletar">delete</span>
                        </button>
                    </form>
                </div>
            </div>
    ...
    <div id="modal-excluir"
        style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.5); z-index:9999; align-items:center; justify-content:center; padding:1em;">
        <div
            style="background:white; border-radius:16px; padding:2em 1.5em; max-width:380px; width:100%; text-align:center; position:relative;">

            <div
                style="width:64px; height:64px; border-radius:50%; background:#fef2f2; display:flex; align-items:center; justify-content:center; margin:0 auto 1em;">
    ...
    <script>
        ...
        function editarDadosAluno(event, id) {
            if (event.target.dataset.value === 'deletar') return;
            window.location.href = "{{ url('cliente/aluno/' . $aluno->id . '/edit') }}";
        }
