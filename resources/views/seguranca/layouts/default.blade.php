...
<body>
        ...
        <nav class="navbar navbar-inverse">
            ...
            <div class="container-fluid">
                <div class="navbar-header">
                    ...
                    <a class="navbar-brand" href="{{url('')}}"><img class="pull-left" width="30px" src="{{url('images/logo.png')}}"> {{config('policia.nome')}}</a>
                </div>
                <div id="navbar" class="collapse navbar-collapse">
                    ...
                    <ul class="nav navbar-nav">
                        ...
                        @include('Seguranca::layouts.submenu', ['submenu' => $raiz])
                        @else
                        <li><a href="{{url($raiz['acao'] ? $raiz['acao'] : '#!')}}">{{$raiz['nome']}}</a></li>
                        @endif
                        @endforeach
                    </ul>
                    @endif
                    <ul class="nav navbar-nav pull-right">
                        ...
                        <li class="dropdown">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><i
                                    class="glyphicon glyphicon-cog"></i> {{\Illuminate\Support\Facades\Auth::user()->nome}} <span class="caret"></span></a>
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li><a href="{{route('home')}}">Página inicial</a></li>
                                <li><a href="{{route('configuracoes.usuario')}}">Alterar senha</a></li>
    ...
    <footer class="footer">
        ...
        <div class="container">
            <p class="text-muted">&copy; {{date('Y')}} - {{config('policia.desenvolvido_por')}}</p>
        </div>
    </footer>
