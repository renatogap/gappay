...
import api from "@/api";
import {AcoesFilhas, PerfisAcoes} from "@/types/App";
...
const link = (subItem: AcoesFilhas) => {
...
onMounted(() => {
  ocupado.value = true
  api.get(`/perfil/${props.id}/edit`)
      .then(({data}) => {

        tela.grupos = data.grupos
        form.nome = data.registro.nome
        form.permissoes = data.permissoes_concedidas
        tabela.value = data.destaque
      })
      .finally(() => {
        ocupado.value = false
      });
